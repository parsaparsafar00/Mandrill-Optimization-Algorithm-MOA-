...
class abc:
    ...
    def __init__(self,
        ...
        if (iterations < 1):
            warn_message = 'Using the minimum value of iterations = 1'
            wrn.warn(warn_message, RuntimeWarning)

        self.employed_onlookers_count = int(max([(colony_size / 2), 2]))
        if (colony_size < 4):
            warn_message = 'Using the minimum value of colony_size = 4'
            wrn.warn(warn_message, RuntimeWarning)
...
def main():
    ...
    dim = 10
    obj_func = F40(dim)
    boundaries = np.array([(-5, 5) for _ in range(dim)])  # define number of iterations
    abc_obj = abc(obj_func,
                  boundaries,
                  colony_size=50,
                  scouts=0.5,
                  iterations=500,
